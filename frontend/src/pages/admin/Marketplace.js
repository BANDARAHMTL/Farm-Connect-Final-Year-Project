...
function MarketplaceModal({ listing, mills, onClose, onSave }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Send form fields - backend accepts both camelCase and snake_case
      fd.append("millId", form.millId);
      fd.append("riceTypeId", form.riceTypeId);
      fd.append("title", form.title);
      fd.append("pricePerKg", form.pricePerKg);
      fd.append("availableKg", form.availableKg);
      fd.append("minOrderKg", form.minOrderKg);
      fd.append("maxOrderKg", form.maxOrderKg);
          ...
          <div style={s.grid2}>
            <div>
              ...
              <select style={s.input} value={form.millId || ""} onChange={e => setForm({ ...form, millId: e.target.value, riceTypeId: "" })} required>
                ...
                {mills.map(m => <option key={m.id} value={m.id}>{m.mill_name || m.millName}</option>)}
              </select>
            </div>
            <div>
...
export default function Marketplace() {
  ...
  async function loadAll() {
    try {
      // Load rice mills (for dropdown in form)
      ...
      try {
        ...
        console.warn("⚠️ Could not load marketplace listings:", marketError.message);
      }
      ...
      <table style={s.table}>
        ...
        <tbody>
          {filtered.map(l => (
            <tr key={l.id}>
              <td style={s.td}>
                {l.imageUrl || l.image_url ? (
                  <img
                    src={(l.imageUrl || l.image_url).startsWith("http") ? (l.imageUrl || l.image_url) : BASE + (l.imageUrl || l.image_url)}
                    alt={l.title}
              ...
              <td style={s.td}>{l.millName || "N/A"}</td>
              <td style={s.td}><span style={{ background: "#f0f9ff", color: "#0369a1", padding: "2px 8px", borderRadius: 12, fontSize: "0.78rem" }}>{l.riceTypeName || "N/A"}</span></td>
              <td style={s.td}><strong style={{ color: "var(--g700)" }}>Rs {typeof l.pricePerKg === 'number' ? Number(l.pricePerKg).toLocaleString() : "N/A"}</strong></td>
              <td style={s.td}>{typeof l.availableKg === 'number' ? Number(l.availableKg).toLocaleString() : "N/A"} kg</td>
              <td style={s.td}>{l.deliveryTime || "N/A"}</td>
              <td style={s.td}><span style={s.badge(l.status)}>{l.status}</span></td>
              <td style={s.td}>
                <div style={{ display: "flex", gap: 6 }}>
                  <button style={s.btn("#f0f4ff", "var(--g700)")} onClick={() => setModal(l)}>Edit</button>
